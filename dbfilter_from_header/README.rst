...
Note that Odoo 19 also ships its own ``X-Odoo-Database`` header, which
selects one exact database rather than filtering with a regular
expression, and makes the session stateless. The two are not
interchangeable, and Odoo refuses a request that carries both an
``X-Odoo-Database`` header and a ``session_id`` cookie. Keep sending
``X-Odoo-dbfilter`` and do not mix the two headers.
